...
import { QuestionEditor } from "../classes/QuestionEditor.js";
...
export class Test_kecerdasan {
    ...
    static open_modal = async (test_id: string, status: number): Promise<void> => {
        ...
        if (!(status == 1)) {
            ...
            console.log(data)
            if (Test_kecerdasan.test_is_start()) Test_kecerdasan.close_question_editor()
    ...
    static add_test_kecerdasan = async (form: HTMLFormElement): Promise<void> => {
    ...
    static clear_kecerdasan_question_input = (): void => {
        Render.TextAll("#kecerdasanDetailModal .editor_questions_input", "")
    }

    static upload_kecerdasan_question = async (): Promise<void> => {
        let question = $$("#kecerdasanDetailModal #single_question_editor .editor_questions_input") as NodeListOf<HTMLInputElement>
        ...
        question.forEach((q: HTMLInputElement, i) => {
            if (i === 0) val.question = q.innerHTML
            else if (i === 1) val.answer = q.value
            else if (i > 1) val.options.push(q.innerHTML)
        })
        let formData = new FormData()
        formData.append('test_id', testKecerdasan.test_id)
    ...
    static add_kecerdasan_question_options = (): void => {
    ...
    static remove_kecerdasan_question_options = (): void => {
