...
import { testKecermatanModal, Test_kecermatan } from "./Test_kecermatan.js";
...
export class QuestionTestDetail {
    static openModal = (test_name: string) => {
        ...
        if (test_is_start) question_detail_modal.set_allow_edit(false)
        else question_detail_modal.set_allow_edit(true)

        if (question_detail_modal.modal_data.length === 0) return alert('tidak ada soal yang akan ditampilkan');
    ...
    static closeModal = (test_name: string) => {
        Render.showElement(`#${test_name}DetailModal #questions_details`, false)
    }

    static update_question = async () => {
        console.log(question_detail_modal.allow_edit)
        if (!question_detail_modal.allow_edit) QuestionTestDetail.openModal(question_detail_modal.test_name)
        ...
        if (json_new_data === json_old_data) return alert("tidak ada perubahan data dari sebelumnya")
        ...
        console.log(data)
        if (question_detail_modal.test_name === "kecerdasan") testKecerdasan.modal_data.questions_list = data.data.questions_list
        else testKepribadian.modal_data.questions_list = data.data.questions_list
