...
let file_question_upload = $("#fileQuestionKepribadianUpload") as HTMLInputElement
...
export class Test_kepribadian {
    ...
    static add_test_kepribadian = async (form: HTMLFormElement): Promise<void> => {
        let formData = new FormData(form)
        let hours: number = parseInt(String(formData.get("duration_hours"))) * 3600
        let seconds: number = parseInt(String(formData.get("duration_minutes"))) * 60
    ...
    static open_question_editor = (mode: string): void => {
        ...
        if (test_is_start) return alert("Anda tidak boleh mengedit test ketika test sedang berjalan")
    ...
    static close_question_editor = (): void => {
        Render.showElement("#kepribadianDetailModal #question_editor", false)
        Render.showElement(`#kepribadianDetailModal #questions_details`, false)
    }
    static clear_kepribadian_question_input = (): void => {
        Render.TextAll("#kepribadianDetailModal .editor_questions_input", "")
    }

    static upload_kepribadian_question = async (): Promise<void> => {
        let question = $$("#kepribadianDetailModal #single_question_editor .editor_questions_input") as NodeListOf<HTMLInputElement>
        let val: any = {
            q_id: Utility.GenerateID(5),
            options: [],
            options_score: []
        ...
        try {
            question.forEach((q: HTMLInputElement, i) => {
                if (i === 0) {
                    console.log(q.innerHTML)
                    if (!q.hasChildNodes()) throw "Pertanyaan Tidak Boleh Kosong"
                    val.question = q.innerHTML
                }
                else if (i === 1) {
                    if (q.value == "0" || q.value == "") throw "Nilai tidak boleh kosong atau 0"
                    val.max_value = q.value
                }
                else if (i > 1) {
                    if (!q.hasChildNodes()) throw "Opsi tidak boleh kosong"
                    val.options.push(q.innerHTML)
                    let score_el = q.nextElementSibling!.getElementsByClassName("options_score")[0] as HTMLInputElement
                    if (score_el.value == "") throw "Nilai Opsi tidak boleh kosong atau 0"
                    if (parseInt(score_el.value) < 0 || parseInt(score_el.value) > 100) throw "Nilai Opsi Tidak Lebih dari 100 atau kecil dari 0"
            ...
            $$("#kepribadianDetailModal .editor_questions_input").forEach(el => {
                if (el.tagName.toLocaleLowerCase() === "select") return;
                el.innerHTML = ""
            })
    ...
    static remove_kepribadian_question_options = (): void => {
    ...
    static upload_file_question = async (): Promise<void> => {
        let formData = new FormData()
        formData.append('test_id', testKepribadian.test_id)
