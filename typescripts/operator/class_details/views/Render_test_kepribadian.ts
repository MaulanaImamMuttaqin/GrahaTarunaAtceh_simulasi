...
import { KepribadianDetail } from "../types.js";


export class Render_test_kepribadian {
    static test_detail(data: KepribadianDetail) {
        Object.entries(data).forEach(([key, value], index) => {
            let el = $(`#detail_kepribadian_${key}`) as HTMLDivElement
            if (el) {
                if (key === "duration") {
    ...
    static toggle_edit_mode(edit_mode: boolean, data: KepribadianDetail) {
        if (edit_mode) {
