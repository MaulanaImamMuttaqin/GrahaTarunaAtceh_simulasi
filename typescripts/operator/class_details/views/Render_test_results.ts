...
export class Render_test_results {
    static results_table(data: Array<TestResults>) {
        const start = performance.now()
        if (!data) return;

        let tbody = $("#participant_test_result_tbody") as HTMLTableElement
        tbody.innerHTML = ''

        let fragment = document.createDocumentFragment();
        data.forEach((d: TestResults, i: number) => {
            ...
            let tr =
                _("tr", { class: "bg-white border-b dark:bg-gray-800 dark:border-gray-700" },
                    [
                        ...
                        _("td", { class: "py-4 px-6 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400" },
                            _("button", {
    ...
    static show_participant_test_result(test: string): void {
    ...
    static render_result_data(test_name: string, data: any): void {
        if (data[test_name]) {
    ...
    static render_kecermatan_result_data(data: any): void {
    ...
    static render_kecermatan_result_table(data: any): void {
        let tbody = $("#participant_result_table tbody") as HTMLTableElement
        tbody.innerHTML = ''
    ...
    static render_kecerdasan_result_data(data: any): void {
    ...
    static render_resume_result_table(test: string, data: any): void {
        let tbody = $(`#participant_${test}_result_table tbody`) as HTMLTableElement
        tbody.innerHTML = ''
