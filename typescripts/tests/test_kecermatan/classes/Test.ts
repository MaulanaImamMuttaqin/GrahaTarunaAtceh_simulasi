...
import { answer_detail, Configuration, Score, Test_cache } from "./Types.js";
...
export class Test {
    ...
    constructor(config: Configuration) {
        ...
        if (this.test_cache.is_start) this.continueTest()
            ...
            this.numbers_list.forEach((q: string) => {
                this.score.detail?.push({
                    question_number: q,
                    total: 0,
                    wrong: 0,
                    correct: 0,
    ...
    calculateTestResult(): void {
        let tot_diff_total = this.score.detail.reduce((total, each: answer_detail) => {
            return total + each.tot_diff!
        }, 0)

        let total_ketahanan = this.score.detail.reduce((total, each: answer_detail) => {
    ...
    setQuestionStability() {
        if (!(this.question_index > 0)) return;
    ...
    private shuffleChar(string: string): string {
        var a = string.split(""),
            n = a.length,
            rand;
        if (this.index_num_removed.length >= (n / 2)) this.index_num_removed = []
    ...
    private generateNumbers(digits: number, total: number): string[] {
        let n: string[] = []
