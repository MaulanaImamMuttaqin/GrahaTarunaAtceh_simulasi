...
export class Render_test_kepribadian {
    ...
    static toggle_edit_mode(edit_mode, data) {
        if (edit_mode) {
            $(`#detail_kepribadian_duration`).innerHTML = `<input name="duration" id="edit_duration" type='time' class='rounded-lg border border-gray-200 w-24 text-black h-full' value='${Utility.convertToHM(parseInt(data.duration))}'>`;
            $(`#detail_kepribadian_test_start_at`).innerHTML = `<input name="test_start_at" id="edit_test_start_at" type='datetime-local' class='rounded-lg border border-gray-200 w-40 text-xs px-0 pl-2 text-black h-full' value='${data.test_start_at.replace(" ", "T")}'>`;
            $(`#detail_kepribadian_test_end_at`).innerHTML = `<input name="test_end_at" id="edit_test_end_at" type='datetime-local' class='rounded-lg border border-gray-200 w-40 text-xs px-0 pl-2 text-black h-full' value='${data.test_end_at.replace(" ", "T")}'>`;
